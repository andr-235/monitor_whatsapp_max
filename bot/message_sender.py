...
async def send_message_with_media(bot: Bot, chat_id: int, message: MessageView) -> None:
    ...
    if media is None:
        await bot.send_message(chat_id=chat_id, text=format_message(message))
        return

    caption = format_message_caption(message, media.caption)
    try:
        if media.media_type == "image":
            await bot.send_photo(chat_id=chat_id, photo=media.url, caption=caption)
            return
        if media.media_type in {"video", "short"}:
            await bot.send_video(chat_id=chat_id, video=media.url, caption=caption)
            return
        if media.media_type == "gif":
            await bot.send_animation(chat_id=chat_id, animation=media.url, caption=caption)
            return
        if media.media_type == "document":
            await bot.send_document(chat_id=chat_id, document=media.url, caption=caption)
            return
        if media.media_type == "audio":
            await bot.send_audio(chat_id=chat_id, audio=media.url, caption=caption)
            return
        if media.media_type == "voice":
            await bot.send_voice(chat_id=chat_id, voice=media.url, caption=caption)
            return
    ...
    await bot.send_message(chat_id=chat_id, text=format_message(message, force_links=force_links))
